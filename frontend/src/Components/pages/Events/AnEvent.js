    ...
    return (
        <div className="col ">
            <div className="card h-100 ">
                <img src={img} className="card-img-top" style={{maxHeight: 200}} alt="..."/>
                    <div className="card-body position-relative">
                        <span className="text-white bg-danger py-2 px-4 position-absolute bottom-100 start-0" >{id}</span>
                        <h4 className="card-title mb-1" style={{minHeight: 40,color: 'initial'}}>{title}</h4>
                        <div className="text-secondary small mb-2" style={{minHeight: 40}}>
                        <span className="me-4"><BsFillAlarmFill className="me-1"/> From {start_date.split("T")[0]} to {end_date.split("T")[0]}</span>
                       <br/><span><BsFillCursorFill className="me-1"/>{location}</span>

                        </div>
                        <h5 className="text-danger"><BsListStars className="small me-2"/>Event Details:</h5>
                        <p className="card-text overflow-hidden" style={{maxHeight: 50, color:'initial'}}>{details}...</p>
                    </div>
